...
class Classifier(lang: String) {
  ...
  private val _wcount   = mutable.HashMap[Pair[String, String], Int]() // [[category, word], count]
  private val _ccount   = mutable.HashMap[String, Int]()               // [category, count]
  private val _probs    = mutable.HashMap[String, Double]()            // [category, probability]
  ...
  def train(category: String, text: String) {
    ...
    tokenizer.eachWord().foreach(word => _wcount.put((category, word), 
      1+_wcount.getOrElse((category, word), 0)))
    _ccount.put(category, 1+_ccount.getOrElse(category, 0))
  }
